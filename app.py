...
# Two-way Telegram commands (optional, may not exist if user hasn't upgraded)
try:
    from telegram_commands import (
        process_pending_commands as _tg_process_commands,
        send_open_signal as _tg_send_open_signal,
        CommandState as TgCommandState,
    )
    _HAS_TG_COMMANDS = True
except ImportError:
    try:
        from engine.telegram_commands import (
            process_pending_commands as _tg_process_commands,
            send_open_signal as _tg_send_open_signal,
            CommandState as TgCommandState,
        )
        ...
        _tg_process_commands = _tg_send_open_signal = None
        TgCommandState = None
        _HAS_TG_COMMANDS = False
...
with tab_live:
    ...
    if "tg_cmd_state" not in st.session_state and _HAS_TG_COMMANDS:
        st.session_state.tg_cmd_state = TgCommandState()

    # ---- Two-way Telegram: process pending commands + send open signal --
    if _HAS_TG_COMMANDS and tg_is_configured():
        # Build context that handlers can use
        def _get_signal_result():
            ...
            try:
                return evaluate_signal(
                    ...
                    config  =config,
                    cfg     =st.session_state.get("sig_cfg", _LOCALS["signals"].SignalConfig()),
                )
            except Exception:
                return None
        ...
        # Process any pending /commands you sent
        try:
            cmd_results = _tg_process_commands(st.session_state.tg_cmd_state, ctx)
            for cmd_text, ok in cmd_results:
            ...
            st.session_state.tg_history.add(
                f"Command processing failed: {e}", "failed", error=str(e)
            )

        # "Open" signal: when the user first opens the app, push current state
        if (multi_tf_ready
                and st.session_state.get("tg_open_signal_enabled", True)):
            try:
                ...
                st.session_state.tg_history.add(
                    f"Open-signal send failed: {e}", "failed", error=str(e)
                )
...
with tab_alerts:
    ...
    # Two-way commands + open-signal toggle
    st.session_state.tg_open_signal_enabled = st.checkbox(
        ...
        key="tg_open_signal_enabled",
        help="When you open the app, automatically send the current multi-TF signal "
             "(with chart) to Telegram. Cooldown 30 min so it doesn't spam on refresh.",
        disabled=not configured,
    )
    ...
    if _HAS_TG_COMMANDS:
        ...
        st.caption(
            "Send these messages to the bot from your Telegram chat. "
            "The app will pick them up on the next refresh and reply."
        )
        st.markdown(
            """
| Command | What it does |
|---|---|
| `/status`  | Current composite, regime, flow, gold price |
| `/signal`  | Multi-TF signal (BUY/SELL/NOACTION) + probability |
...
| `/help`    | List of commands |
        ...
        st.caption(
            "⏱ Per-command cooldowns: `/status` 60s, `/signal` 60s, `/chart` 120s, "
            "`/weights` 30s, `/help` 10s. The app polls Telegram on every refresh "
            "(every 15 min via auto-refresh)."
        )
    else:
        st.warning(
            "Two-way commands module (`telegram_commands.py`) is not loaded. "
            "Make sure the file is at the repo root and refresh the app."
        )
