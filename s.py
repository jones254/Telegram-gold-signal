...
# -----------------------------------------------------------------------------
def signal_value(asset_scores: Dict[str, float],
                 weights: Optional[Dict[str, float]] = None) -> float:
    """
    signal = (((DXY + VIX) * -1) + SP500) * 2

    Each market's contribution is its asset score (post-flip) multiplied
    by its user-configured weight.  Weights come from `config.weights`
    (the same weights the COMPOSITE uses).  If `weights` is None, raw
    scores are used (legacy behavior).
    ...
    The final × 2 amplifies the signal after weighting.  This is the
    scale factor you specified in the original spec.
    """
    def _contrib(mkt: str) -> float:
        v = asset_scores.get(mkt, 0.0)
        if weights is not None and mkt in weights:
            v = v * weights[mkt]
        return v
...
def evaluate(
    ...
    sig_5m  = signal_value(as_5m,  config.weights)
    sig_15m = signal_value(as_15m, config.weights)
    sig_1h  = signal_value(as_1h,  config.weights)
